	...
	const FVector& CurrentVelocity) const
{
	...
	//Dont override vertical velocity if in falling physics, we want to keep the effect of gravity
	if(IsFalling())
	{
		const FVector GravityDirection = GetGravityDirection(true);
	...
	const FVector& Delta, const FVector& subLoc)
{
	...
	if(bFallingRemoveSpeedZ && !GravityDirection.IsZero())
	{
	...
	if (IsMovingOnGround())
	{
		//don't set MOVE_Falling straight away because when loaded, the player may fell off the map
			...
			else
			{
				...
				if (SubTimeTickRemaining > KINDA_SMALL_NUMBER && (Delta | Adjusted) > 0.0f)
				{
					...
					if (Hit.bBlockingHit)
					{
						...
						if (bHasAirControl && (Hit.Normal | GravityDir) < -GVertical_Slope_Normal_Z)
						{
	...
	bool bZeroDelta, const FHitResult* DownwardSweepResult) const
{
	...
	const float FloorSweepTraceDist = FMath::Max(MAX_FLOOR_DIST, MaxStepHeight + HeightCheckAdjust);
	...
	FStepDownResult* OutStepDownResult)
{
	...
	if (SweepHit.IsValidBlockingHit())
	{
		...
		// Reject un walkable surface normals here.
		if (!IsWalkable(SweepHit))
		{
			// Reject if normal opposes movement direction.
			if ((Delta | SweepHit.ImpactNormal) < 0.0f)
			{
				ScopedStepUpMovement.RevertMove();
				return false;
			}
...
void USCharacterMovementComponent::SetDefaultMovementMode()
{
	...
	else if (!CharacterOwner || MovementMode != DefaultLandMovementMode)
	{
		...
		if (MovementMode == MOVE_Walking && GetMovementBase() == nullptr)
		{
			SetMovementMode(MOVE_Falling);
		}
	}
...
void USCharacterMovementComponent::HandleImpact(const FHitResult& Hit, float TimeSlice, const FVector& MoveDelta)
{
	...
	if (APawn* OtherPawn = Cast<APawn>(Hit.GetActor()))
	{
	...
	if (IsMovingOnGround())
	{
		...
		else if (Dot < -KINDA_SMALL_NUMBER)
		{
			...
			if (CurrentFloor.FloorDist < MIN_FLOOR_DIST && CurrentFloor.bBlockingHit)
			{
				...
				if ((Delta | FloorNormal) < 0.0f && (FloorNormal | CapsuleUp) < 1.0f - DELTA)
				{
	...
	while (RemainingTime >= MIN_TICK_TIME && Iterations < MaxSimulationIterations && CharacterOwner && (CharacterOwner->Controller || bRunPhysicsWithNoController || HasAnimRootMotion()))
	{
		...
		if (bCheckLedges && !CurrentFloor.IsWalkableFloor())
		{
			...
			else
			{
				...
				bool bMustJump = bZeroDelta || OldBase == nullptr || (!OldBase->IsCollisionEnabled() && MovementBaseUtility::IsDynamicBase(OldBase));
